...
++  skim-sotx
  ...
  +$  single
    $%  $:  %spawn  =pass
            ::from=(unit [=pos =off])
            to=[spkvh=@ux pos=(unit pos) =off tej=off]
        ==
        [%keys =pass breach=?]
        [%escape parent=ship]
...
++  parse-roll
  ...
  ::
  ?~  parse-result
    ~&  parse-failed=num-msgs
    (debug %parse-failed ~)
  ~&  parse-success=num-msgs
...
++  parse-raw-tx
  ...
  |^
  ~&  %try-parse-raw-tx
  =/  res=(unit [tx=sotx cur=@ud])  parse-tx
  ?~  res  ~&  %parse-tx-failed  ~
  ~&  %parse-tx-success
  =/  dif  (sub cur.u.res cur)
  =/  len  =>((dvr dif 8) ?:(=(0 q) p +(p)))
  :-  ~  :_  cur.u.res
  [[len (cut 0 [cur dif] batch)] tx.u.res]
  ::[sig [len (cut 0 [cur dif] batch)] tx.u.res]
  ::
  ++  parse-tx
    ^-  (unit [sotx cur=@ud])
    ~&  %try-parse-tx
    =^  pad               cur  (take 0 5)
    =/  sig  take-sig
    ?~  sig  ~&  $no-sig  (debug %no-sig ~)
    =^  sig  cur  u.sig
    =^  from-ship=ship    cur  (take 0 128)
    =-  ?~  res
          ~
        `[[[from-ship sig] skim-sotx.u.res] cur.u.res]
    |-  ^-  res=(unit [=skim-sotx cur=@ud])
    =^  op   cur  (take 0 7)
    ~&  op=op
    ...
    ::
      ...
      ::
      ++  take-to
        ^-  (unit [[spkvh=@ux pos=(unit pos) =off tej=off] cur=@])
        =^  spkvh  cur  (take 0 256)
        =^  off    cur  take-atom
        =^  tej    cur  take-atom
        =^  pos-o  cur  (take 0 2)
        ~&  pos-o=pos-o
...
++  en
  ...
  ++  rip-octs
    ...
    ^-  (list octs)
    =/  zero-520s=(list octs)
      ?:(=(p.nzeros 0) ~ (turn (gulf 1 p.nzeros) |=(* [520 0])))
  ...
  ++  handle-tx
    ...
    ++  check-for-unv
      ...
      ?:  ?=(%spawn -.sot)
        ...
        ?~  sont=(get-spawn-sont +>.sot)  $(sots t.sots)
        =/  sponsor  `@p`(end 4 our)
        =/  =^point
          :*  own=[u.sont ~]
              rift=0
        ...
        %_    $
            point    `point
            sots     t.sots
            sont-map  (put-com:si sont-map txid.u.sont pos.u.sont off.u.sont our)
            unv-ids   (~(put by unv-ids) our point)
            fx
          :*  [%point our %owner u.sont]
