...
++  rpc
  ...
  =>  =,  abi
      ...
      ++  event-log
        $:  ::  null for pending logs
            $=  mined  %-  unit
            $:  trace=(unit raw-trace:eth-trace)
                tx=(unit [from=@ux to=(unit @ux) input=@ux])
                log-index=@ud
  ...
  ++  request-to-json
    ...
    ::
      ...
      ['eth_sendRawTransaction' (tape (num-to-hex dat.req)) ~]
        %trace-transaction
      ['trace_transaction' (tape (num-to-hex txh.req)) ~]
    ::
        %trace-filter
      ['trace_filter' (trace-filter-to-json +.req) ~]
    ==
  ::
  ++  eth-call-to-json
    =,  enjs:format
  ...
  ++  trace-filter-to-json
    ...
    ^-  (list (unit (pair @t json)))
    :~  ?~  frob.tf  ~
        `'fromBlock'^(block-to-json u.frob.tf)
      ::
        ?~  tob.tf  ~
        `'toBlock'^(block-to-json u.tob.tf)
      ::
      ?~  froa.tf  ~
        `'fromAddress'^a+(turn froa.tf (corl tape address-to-hex))
      ::
      ?~  toa.tf  ~
        `'toAddress'^a+(turn toa.tf (corl tape address-to-hex))
      ::
      ?~  aft.tf  ~
        `'after'^(tape (num-to-hex u.aft.tf))
      ::
      ?~  cont.tf  ~
        `'count'^(tape (num-to-hex u.cont.tf))
    ==
  ::
  ++  block-to-json
    |=  dob=block
