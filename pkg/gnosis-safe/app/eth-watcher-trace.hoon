...
++  on-load
  ...
  =?  old-state  ?=(%5 -.old-state)
    ...
    %=    old-state
      ...
      %=  dog
          -
        =,  -.dog
        =/  events  ?~(contracts ~ `[contracts topics])
        =/  batchers  ?~(batchers ~ `[batchers topics])
        [url eager refresh-rate timeout-time from events batchers ~]
      ::
          pending-logs-5
        %-  ~(run by pending-logs-5.dog)
        |=  =loglist-5
        %+  turn  loglist-5
        |=  =event-log-5
        =+  event-log-5
        %=    event-log-5
          ...
          ?~  mined  mined
          ?~  input.u.mined  `mined
          `mined(u.input.u [0x0 ~ u.input.u.mined])
        ==
          history-5
        %+  turn  history-5.dog
        |=  =loglist-5
  ...
  +$  pending-logs-5  (map number:block loglist-5)
  +$  history-4       (list loglist-4)
  +$  loglist-4       (list event-log-4)
  +$  history-5       (list loglist-5)
  +$  loglist-5       (list event-log-5)
  +$  event-log-4
    $:  $=  mined  %-  unit
        $:  log-index=@ud
