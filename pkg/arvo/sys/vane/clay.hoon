...
++  load
  ...
  =>  |%
      +$  raft-any
        $%  [%17 raft-17]
            ...
            [%6 raft-6]
        ==
      +$  raft-16-any      ?(raft-16-raw raft-16-old-raw)
      +$  raft-16-raw      [* * * * * * * * * * @ @]
      +$  raft-16-old-raw  [* * * * * * * * * * * @ @]
      +$  raft-17  raft
      ::
  ...
  =?  old  ?=(%16 -.old)  17+(raft-16-to-17 (raft-16-any-lift +.old))
  ?>  ?=(%17 -.old)
  ..^^$(ruf +.old)
  ::
  ...
  ++  raft-15-to-16
    ...
    --
  ::  +raft-16-any-lift: lift $raft-16-any to $raft-16
  ::
  ++  raft-16-any-lift
    |=  raf=raft-16-any
    |^  ^-  raft-16
    ?:  ?=(raft-16-raw raf)
      ;;(raft-16 raf)
    =/  raf-old  ;;(raft-16-old raf)
    =<  [&1 &2 &3 |4]
    %=  raf-old
        dos.rom
      %-  ~(run by dos.rom.raf-old)
      |=  d=dojo-16-old
      d(dom (dome-13-to-dome dom.d))
    ::
        hoy
      %-  ~(run by hoy.raf-old)
      |=  r=rung-16-old
